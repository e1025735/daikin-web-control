...
//Select the first unit as the default unit selection when opening or reloading the page
document.getElementById(1).click();
//Check if the ip adddres of the second unit is still in the unconfigured format "nnn.nnn.nnn.nnn". 
if (document.getElementById("unit-ip-2").getAttribute("data-unit-ip") === "nnn.nnn.nnn.nnn") {
	//NOTE: Hide the unit selection buttons when the GUI is just for one unit. Unit must be configured as the first unit in configuration section in the index.html 
	document.getElementById("unit-row").style="display: none;";
}

function request_control() {
	var ip = getActiveUnit_IP();
	var target="../daikinControl/api.php";
	var request="GET";
	...
	xmlhttp.onreadystatechange  = function () {
		if ( xmlhttp.readyState == 4 ){
			...
			if((! request_control_loading) && (! request_sensor_loading)){set_loading(1);}
...
function temp_onclick(inc){
	...
	send_control(temp);
	//document.getElementById("target_temp").style="background-color:lightgray";
	update();
}
...
function set_home_temp(temp){
	document.getElementById("home_temp").innerHTML=" "+temp+degrees+"C";	
}

function set_outside_temp(temp){
	document.getElementById("outside_temp").innerHTML=" "+temp+degrees+"C";
}
...
function set_target_temp(temp){
	if(isNaN(temp)){
		...
		document.getElementById("target_temp").innerHTML=" "+temp+degrees+"C";
...
function getActiveUnit_IP(){
	//Return the IP address of the selected (active) unit based on the class btn-info set on the activated unit button
	var unitId = document.getElementsByClassName("btn btn-info unit-btn")[0].getAttribute("id");
	var unitName = document.getElementById("unit-name-" + unitId).getAttribute("data-unit-name")
	document.getElementById("navbar-brand").innerHTML="AC Control " + unitName;
	return document.getElementById("unit-ip-" + unitId).getAttribute("data-unit-ip");
	}
